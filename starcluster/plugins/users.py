...
class CreateUsers(clustersetup.DefaultClusterSetup):
    ...
    def run(self, nodes, master, user, user_shell, volumes):
        ...
        current_batch_file_users = self._get_newusers_batch_file(master, self._usernames,
                                                 user_shell)
	...
	for user in new_users:
		if current_user_map.has_key(user):		
	...
	new_batch_file = self._get_newusers_batch_file(master, batchfile_new_users, user_shell, add_new_users=True)
        ...
        log.info("Configuring passwordless ssh for %d cluster users" % self._num_users)
        pbar = self.pool.progress_bar.reset()
        pbar.maxval = self._num_users

        for i, user in enumerate(self._usernames):
            master.generate_key_for_user(user, auth_new_key=True, auth_conn_key=True)
