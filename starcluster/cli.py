...
class CmdLoadBalance(CmdBase):
    ...
    def addopts(self, parser):
        ...
        opt = parser.add_option("-m","--max_nodes",dest="max_nodes",
            action="store", type="int", default=None,
            help="Maximum # of nodes in cluster")
        
        opt = parser.add_option("-w","--job_wait_time",dest="wait_time",
            action="store", type="int", default=None,
            help="Maximum wait time for a job before adding nodes, seconds")
        
        opt = parser.add_option("-a","--add_nodes_per_iter",dest="add_pi",
            action="store",type="int",default=None,
            help="Number of nodes to add per iteration")

        opt = parser.add_option("-k","--kill_after", dest="kill_after",
            action="store",type="int",default=None,
            help="Minutes after which a node can be killed")

        opt = parser.add_option("-s","--stabilization_time", dest="stab",
            action="store",type="int",default=None,
            help="Seconds to wait before cluster stabilizes")

        opt = parser.add_option("-l","--lookback_window", dest="lookback_win",
            action="store",type="int",default=None,
    ...
    def execute(self, args):
        ...
        plot = self.opts.plot
        lb = sge.SGELoadBalancer(cluster_tag, self.cfg, **self.specified_options_dict)
        lb.run()
